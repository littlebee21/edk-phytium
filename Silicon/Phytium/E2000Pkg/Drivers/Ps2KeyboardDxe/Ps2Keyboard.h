...
#define writeb(v,c)	({ UINT8  __v = v; __iowmb(); __arch_putb(__v,c); __v; })
#define writew(v,c)	({ UINT16 __v = v; __iowmb(); __arch_putw(__v,c); __v; })
#define writel(v,c)	({ UINT32 __v = v; __iowmb(); __arch_putl(__v,c); __v; })
#define writeq(v,c)	({ UINT64 __v = v; __iowmb(); __arch_putq(__v,c); __v; })

#define readb(c)	({ UINT8  __v = __arch_getb(c); __iormb(); __v; })
#define readw(c)	({ UINT16 __v = __arch_getw(c); __iormb(); __v; })
#define readl(c)	({ UINT32 __v = __arch_getl(c); __iormb(); __v; })
#define readq(c)	({ UINT64 __v = __arch_getq(c); __iormb(); __v; })

#define LPC_MEM_BASE                    0x20000000

#define SIO_INDEX_PORT                  0x4E | LPC_MEM_BASE
#define SIO_DATA_PORT                   0x4F | LPC_MEM_BASE

#define KEYBOARD_8042_DATA_REGISTER     0x60 | LPC_MEM_BASE
#define KEYBOARD_8042_STATUS_REGISTER   0x64 | LPC_MEM_BASE
#define KEYBOARD_8042_COMMAND_REGISTER  0x64 | LPC_MEM_BASE
