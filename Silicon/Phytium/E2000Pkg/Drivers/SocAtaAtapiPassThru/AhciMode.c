...
/**
  ...
  @param  Instance     The Atapi pass thru protocol instance.
  ...
  )
{
  ...
  Data = MmioRead32 (Instance->RegBase + Offset);
  ...
  )
{
  ...
  MmioWrite32 (Instance->RegBase + Offset, Data);
  ...
  )
{
  ...
  Data  = AhciReadReg (Instance, Offset);

  Data &= AndData;

  AhciWriteReg (Instance, Offset, Data);
}
  ...
  )
{
  ...
  do {
    ...
    //
    Value = AhciReadReg (Instance, (UINT32) Offset) & MaskValue;
  ...
  )
{
  ...
  AhciWriteReg (Instance, Offset, AhciReadReg (Instance, Offset));
  ...
  //
  AhciWriteReg (Instance, EFI_AHCI_IS_OFFSET, AhciReadReg (Instance, EFI_AHCI_IS_OFFSET));
}
  ...
  )
{
  ...
  if (AtaStatusBlock != NULL) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      Data   = AhciReadReg (Instance, (UINT32)Offset);
  ...
  )
{
  ...
  AhciOrReg (Instance, Offset, EFI_AHCI_PORT_CMD_FRE);
  ...
  )
{
  ...
  AhciAndReg (Instance, Offset, (UINT32)~(EFI_AHCI_PORT_CMD_FRE));
  ...
  )
{
  ...
  if (AtapiCommand != NULL) {
    ...
    AhciOrReg (Instance, Offset, (EFI_AHCI_PORT_CMD_DLAE | EFI_AHCI_PORT_CMD_ATAPI));
  } else {
    AhciAndReg (Instance, Offset, (UINT32)~(EFI_AHCI_PORT_CMD_DLAE | EFI_AHCI_PORT_CMD_ATAPI));
  }
   ...
   )
{
  ...
  do {
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SERR;
    if (AhciReadReg(Instance, Offset) != 0) {
      AhciWriteReg (Instance, Offset, AhciReadReg(Instance, Offset));
    }
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_TFD;

    Data = AhciReadReg (Instance, Offset) & EFI_AHCI_PORT_TFD_MASK;
    if (Data == 0) {
      break;
    }
...
/**
  ...
  @param[in] Instance The Atapi pass thru protocol instance.
  @param[in] Port     SATA port index on which to reset.
  ...
  )
{
  ...
  AhciOrReg (Instance, Offset, EFI_AHCI_PORT_SCTL_DET_INIT);
  //
  // SW is required to keep DET set to 0x1 at least for 1 milisecond to ensure that
  // at least one COMRESET signal is sent.
  //
  MicroSecondDelay(1000);
  AhciAndReg (Instance, Offset, ~(UINT32)EFI_AHCI_PORT_SSTS_DET_MASK);

  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SSTS;
  Status = AhciWaitMmioSet (Instance, Offset, EFI_AHCI_PORT_SSTS_DET_MASK, EFI_AHCI_PORT_SSTS_DET_PCE, ATA_ATAPI_TIMEOUT);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  return AhciWaitDeviceReady (Instance, Port);
}
  ...
  )
{
  ...
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_IS;
  PortInterrupt = AhciReadReg (Instance, Offset);
  ...
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CMD;
  AhciAndReg (Instance, Offset, ~(UINT32)EFI_AHCI_PORT_CMD_ST);

  Status = AhciWaitMmioSet (Instance, Offset, EFI_AHCI_PORT_CMD_CR, 0, ATA_ATAPI_TIMEOUT);
  if (EFI_ERROR (Status)) {
  ...
  //
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_TFD;
  PortTfd = AhciReadReg (Instance, Offset);
  if ((PortTfd & (EFI_AHCI_PORT_TFD_BSY | EFI_AHCI_PORT_TFD_DRQ)) != 0) {
    Status = AhciResetPort (Instance, Port);
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (((FisType == SataFisD2H) && ((PortInterrupt & EFI_AHCI_PORT_IS_DHRS) != 0)) ||
    ...
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_TFD;
    PortTfd = AhciReadReg (Instance, (UINT32) Offset);
  ...
  )
{
  ...
  do {
    Status = AhciCheckFisReceived (Instance, Port, FisType);
  ...
  )
{
  ...
  for (Retry = 0; Retry < AHCI_COMMAND_RETRIES; Retry++) {
    ...
    if (Read && (AtapiCommand == 0)) {
      Status = AhciWaitUntilFisReceived (Instance, Port, Timeout, SataFisPioSetup);
      ...
      Status = AhciWaitUntilFisReceived (Instance, Port, Timeout, SataFisD2H);
    }

    if (Status == EFI_DEVICE_ERROR) {
      DEBUG ((DEBUG_ERROR, "PIO command failed at retry %d\n", Retry));
      Status = AhciRecoverPortError (Instance, Port);
      if (EFI_ERROR (Status)) {
        break;
      }
    } else {
  ...
  AhciDumpPortStatus (Instance, AhciRegisters, Port, AtaStatusBlock);
  ...
  )
{
  ...
  if (Task == NULL) {
    ...
    if (Task->IsStart) {
      Status = AhciCheckFisReceived (Instance, Port, SataFisD2H);
      if (Status == EFI_DEVICE_ERROR) {
        DEBUG ((DEBUG_ERROR, "DMA command failed at retry: %d\n", Task->RetryTimes));
        Status = AhciRecoverPortError (Instance, Port);
        //
...
/**
  ...
  @param[in]  Instance           The ATA_ATAPI_PASS_THRU_INSTANCE protocol instance.
  @param[in]  Port               The number of port.
  @param[in]  Timeout            The timeout value of stop, uses 100ns as a unit.
...
/**
  ...
  @param[in]  Timeout            The timeout value of start, uses 100ns as a unit.
  ...
  )
{
  ...
  //
  Capability = AhciReadReg(Instance, EFI_AHCI_CAPABILITY_OFFSET);
  ...
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CMD;
  PortStatus = AhciReadReg (Instance, Offset);

  StartCmd = 0;
  if ((PortStatus & EFI_AHCI_PORT_CMD_ALPE) != 0) {
    StartCmd = AhciReadReg (Instance, Offset);
  ...
  if ((PortTfd & (EFI_AHCI_PORT_TFD_BSY | EFI_AHCI_PORT_TFD_DRQ)) != 0) {
    if ((Capability & BIT24) != 0) {
      Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CMD;
      AhciOrReg (Instance, Offset, EFI_AHCI_PORT_CMD_CLO);
  ...
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CMD;
  AhciOrReg (Instance, Offset, EFI_AHCI_PORT_CMD_ST | StartCmd);

  //
  // Setting the command
  //
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CI;
  AhciAndReg (Instance, Offset, 0);
  AhciOrReg (Instance, Offset, CmdSlotBit);
...
/**
  ...
  @param[in]  Instance           The ATA_ATAPI_PASS_THRU_INSTANCE protocol instance.
  @param[in]  Timeout            The timeout value of reset, uses 100ns as a unit.
  ...
  )
{
  ...
  //
  Value = AhciReadReg(Instance, EFI_AHCI_GHC_OFFSET);

  if ((Value & EFI_AHCI_GHC_ENABLE) == 0) {
    AhciOrReg (Instance, EFI_AHCI_GHC_OFFSET, EFI_AHCI_GHC_ENABLE);
  }

  AhciOrReg (Instance, EFI_AHCI_GHC_OFFSET, EFI_AHCI_GHC_RESET);
...
/**
  ...
  @param  Instance            The ATA_ATAPI_PASS_THRU_INSTANCE protocol instance.
  @param  AhciRegisters       The pointer to the EFI_AHCI_REGISTERS.
  @param  Port                The number of port.
  ...
  )
{
  ...
  if (Instance == NULL || AhciRegisters == NULL || Buffer == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  if (Instance == NULL || AhciRegisters == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  PortImplementBitMap  = AhciReadReg(Instance, EFI_AHCI_PI_OFFSET);
  //
  ...
  ZeroMem ((VOID*)Buffer, (UINTN)MaxReceiveFisSize);

  AhciRegisters->AhciRFis          = (VOID*)Buffer;
  ...
  ZeroMem ((VOID*)Buffer, (UINTN)MaxCommandListSize);

  AhciRegisters->AhciCmdList        = (VOID*)Buffer;
  ...
  //}
  //
  AhciCmdListPciAddr = (UINT64)Buffer;
  ...
  ZeroMem ((VOID*)Buffer, (UINTN)MaxCommandTableSize);

  AhciRegisters->AhciCommandTable    = (VOID*)Buffer;
  ...
  //  goto Error2;
  //}
  AhciCommandTablePciAddr = (UINT64) Buffer;
...
//Error2:
  ...
  //         );
  gBS->FreePages (
         (UINT64) AhciRegisters->AhciCommandTable,
         EFI_SIZE_TO_PAGES ((UINTN) MaxCommandTableSize)
         );
Error3:
  //PciIo->Unmap (
...
//Error4:
  ...
  //         );
  gBS->FreePages (
         (UINT64) AhciRegisters->AhciCmdList,
         EFI_SIZE_TO_PAGES ((UINTN) MaxCommandListSize)
         );
Error5:
  //PciIo->Unmap (
  ...
  )
{
  ...
  //
  Capability2 = AhciReadReg (Instance, AHCI_CAPABILITY2_OFFSET);
  ...
  PortCmd    = AhciReadReg (Instance, Offset + EFI_AHCI_PORT_CMD);
  PortDevSlp = AhciReadReg (Instance, Offset + AHCI_PORT_DEVSLP);
  ...
  if ((IdentifyData->AtaData.serial_ata_features_enabled & BIT8) != 0) {
    Status = AhciDeviceSetFeature (
      Instance, AhciRegisters, Port, 0, ATA_SUB_CMD_ENABLE_SATA_FEATURE, 0x09, ATA_ATAPI_TIMEOUT
    );
  ...
  Status = AhciReadLogExt(Instance, AhciRegisters, Port, PortMultiplier, LogData, 0x30, 0x08);

  //
  // Clear PxCMD.ST and PxDEVSLP.ADSE before updating PxDEVSLP.DITO and PxDEVSLP.MDAT.
  //
  AhciWriteReg (Instance, Offset + EFI_AHCI_PORT_CMD, PortCmd & ~EFI_AHCI_PORT_CMD_ST);
  PortDevSlp &= ~AHCI_PORT_DEVSLP_ADSE;
  AhciWriteReg (Instance, Offset + AHCI_PORT_DEVSLP, PortDevSlp);
  ...
  AhciOrReg (Instance, Offset + AHCI_PORT_DEVSLP, PortDevSlp);
  ...
  AhciWriteReg (Instance, Offset + EFI_AHCI_PORT_CMD, PortCmd);
  ...
  )
{
  ...
  if (IdentifyData->AtaData.specific_config == ATA_SPINUP_CFG_REQUIRED_IDD_INCOMPLETE) {
    ...
    //
    Status = AhciDeviceSetFeature (
               Instance, AhciRegisters, Port, PortMultiplier,
  ...
  Status = AhciIdentify (Instance, AhciRegisters, Port, PortMultiplier, IdentifyData);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (mAtaAtapiPolicy->PuisEnable == 0) {
    Status = AhciDeviceSetFeature (Instance, AhciRegisters, Port, PortMultiplier, ATA_SUB_CMD_DISABLE_PUIS, 0x00, ATA_ATAPI_TIMEOUT);
  } else if (mAtaAtapiPolicy->PuisEnable == 1) {
    Status = AhciDeviceSetFeature (Instance, AhciRegisters, Port, PortMultiplier, ATA_SUB_CMD_ENABLE_PUIS, 0x00, ATA_ATAPI_TIMEOUT);
  }
  ...
  )
{
  ...
  Status = AhciReset (Instance, EFI_AHCI_BUS_RESET_TIMEOUT);
  ...
  Status = AhciCreateTransferDescriptor (Instance, AhciRegisters);
  ...
  for (Port = 0; Port < EFI_AHCI_MAX_PORTS; Port ++) {
    if ((PortImplementBitMap & (((UINT32)BIT0) << Port)) != 0) {
      ...
      AhciWriteReg (Instance, Offset, Data64.Uint32.Lower32);
      Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_FBU;
      AhciWriteReg (Instance, Offset, Data64.Uint32.Upper32);
      ...
      if ((Data & EFI_AHCI_PORT_CMD_CPD) != 0) {
        AhciOrReg (Instance, Offset, EFI_AHCI_PORT_CMD_POD);
      }

      if ((Capability & EFI_AHCI_CAP_SSS) != 0) {
        AhciOrReg (Instance, Offset, EFI_AHCI_PORT_CMD_SUD);
      }
      ...
      //
      Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SCTL;
      AhciOrReg (Instance, Offset, EFI_AHCI_PORT_SCTL_IPM_INIT);
      //
      ...
      Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SSTS;
      do {
        Data = AhciReadReg (Instance, Offset) & EFI_AHCI_PORT_SSTS_DET_MASK;
      ...
      if (PhyDetectDelay == 0) {
        ...
        //
        Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_CMD;
        AhciAndReg (Instance, Offset, (UINT32) ~(EFI_AHCI_PORT_CMD_SUD));
        continue;
      }

      Status = AhciWaitDeviceReady (Instance, Port);
      if (EFI_ERROR (Status)) {
        continue;
      }
      ...
      if ((Data & EFI_AHCI_ATAPI_SIG_MASK) == EFI_AHCI_ATAPI_DEVICE_SIG) {
        Status = AhciIdentifyPacket (Instance, AhciRegisters, Port, 0, &Buffer);
        ...
        Status = AhciIdentify (Instance, AhciRegisters, Port, 0, &Buffer);
      ...
      Status = AhciDeviceSetFeature (Instance, AhciRegisters, Port, 0, 0x03, (UINT32)(*(UINT8 *)&TransferMode), ATA_ATAPI_TIMEOUT);
      if (EFI_ERROR (Status)) {
