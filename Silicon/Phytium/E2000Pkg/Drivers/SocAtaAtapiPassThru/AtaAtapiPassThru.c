  ...
  )
{
  ...
  Status = gBS->OpenProtocol (
                  Controller,
                  &gEfiPhytiumSocIdeControllerInitProtocolGuid,
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Open NonDiscoverableDeviceProtocol Error, Status=%r\n", Status));
  }
  //
  ...
  Instance->RegBase                = Dev->Resources[0].AddrRangeMin;
  ...
  )
{
  ...
  //
  //Pre-allocated resource for AHCI initialization should bu released.
  //
  ...
  )
{
  ...
  switch (Mode) {
    ...
    case EfiAtaAhciMode:
      ...
      Status = AhciPacketCommandExecute (Instance, &Instance->AhciRegisters, Port, PortMultiplier, Packet);
      break;
    default :
      Status = EFI_DEVICE_ERROR;
      break;
