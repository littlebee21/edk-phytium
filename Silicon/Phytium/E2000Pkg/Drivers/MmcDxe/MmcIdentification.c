  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Get Emmc Host Bus Width : %d\n", PcdGet32 (PcdEmmcBusWidth)));
  switch (PcdGet32 (PcdEmmcBusWidth)) {
  case 4:
  case 8:
  case 1:
    Value = PcdGet32 (PcdEmmcBusWidth);
    break;
  default:
    Value = 4;
    break;
  ...
  )
{
  ...
  Argument = EMMC_CMD6_ARG_ACCESS(3) | EMMC_CMD6_ARG_INDEX(ExtCmdIndex) |
             EMMC_CMD6_ARG_VALUE(Value) | EMMC_CMD6_ARG_CMD_SET(0);
  ...
  )
{
  ...
  if (MMC_HOST_HAS_SETIOS(Host)) {
    ...
    Status = EmmcSetEXTCSD (MmcHostInstance, EXTCSD_BUS_WIDTH, EMMC_BUS_WIDTH_4BIT);
    if (EFI_ERROR (Status)) {
  ...
  Status = Host->SendCommand (Host, MMC_CMD8 | CMD_DATA | CMD_READ, 0);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  for (Idx = 0; Idx < 4; Idx++) {
    ...
    Status = Host->SetIos (Host, BusClockFreq, GetEmmcHostBusWidth(), TimingMode[Idx]);
    if (!EFI_ERROR (Status)) {
      switch (TimingMode[Idx]) {
      case EMMCHS52DDR1V2:
      case EMMCHS52DDR1V8:
      ...
      case EMMCHS26:
        ...
        switch (GetEmmcHostBusWidth()) {
        case 8:
          BusMode = EMMC_BUS_WIDTH_8BIT;
          break;
        case 1:
          BusMode = EMMC_BUS_WIDTH_1BIT;
          break;
        default:
          BusMode = EMMC_BUS_WIDTH_4BIT;
          break;
  ...
  )
{
  ...
  /* SD Switch, Mode:0, Group:0, Value:0 */
    ...
    Status = MmcHost->SendCommand (MmcHost, MMC_CMD6 | CMD_DATA | CMD_READ, CmdArg);
    if (EFI_ERROR (Status)) {
  ...
  if (Scr.SD_BUS_WIDTHS & SD_BUS_WIDTH_4BIT) {
    ...
    /* Width: 4 */
    Status = MmcHost->SendCommand (MmcHost, MMC_CMD6 | CMD_NO_DATA, 2);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a (MMC_CMD6): Error and Status = %r\n", __FUNCTION__, Status));
      return Status;
    }
  ...
  if (MMC_HOST_HAS_SETIOS(MmcHost)) {
    ...
    //Status = MmcHost->SetIos (MmcHost, Speed, 1, EMMCBACKWARD);
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (MmcHostInstance->State == MmcHwInitializationState) {
    ...
    MmcHost->SetIos (MmcHost, 400000, 1, EMMCHS52DDR1V8);
  }
  ...
  Status = MmcHost->SendCommand (MmcHost, MMC_CMD8 | CMD_NO_DATA, CmdArg);
  if (Status == EFI_SUCCESS) {
