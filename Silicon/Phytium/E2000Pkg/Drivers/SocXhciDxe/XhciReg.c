...
#endif
  Data = MmioRead8 (Xhc->RegBase + Offset);
...
#endif
  Data = MmioRead32 (Xhc->RegBase + Offset);
  DEBUG ((EFI_D_INFO, "Reg : %x, Data : %x\n", Xhc->RegBase + Offset, Data));
  return Data;
}
...
#endif
  ASSERT (Xhc->CapLength != 0);
  Data = MmioRead32 (Xhc->RegBase + Xhc->CapLength + Offset);
  return Data;
}
...
#endif
  ASSERT (Xhc->CapLength != 0);
  MmioWrite32 (Xhc->RegBase + Xhc->CapLength + Offset, Data);
}
...
#endif
  ASSERT (Xhc->DBOff != 0);
  MmioWrite32 (Xhc->RegBase + Xhc->DBOff + Offset, Data);
}
...
#endif
  ASSERT (Xhc->RTSOff != 0);
  Data = MmioRead32 (Xhc->RegBase + Xhc->RTSOff + Offset);
  return Data;
}
...
#endif
  ASSERT (Xhc->RTSOff != 0);
  MmioWrite32 (Xhc->RegBase + Xhc->RTSOff + Offset, Data);
}
...
#endif
  ASSERT (Xhc->ExtCapRegBase != 0);
  Data = MmioRead32 (Xhc->RegBase + Xhc->ExtCapRegBase + Offset);
  return Data;
}
...
#endif
  ASSERT (Xhc->ExtCapRegBase != 0);
  MmioWrite32 (Xhc->RegBase + Xhc->ExtCapRegBase + Offset, Data);
}
  ...
  )
{
  ...
  if ((Xhc->DebugCapSupOffset == 0xFFFFFFFF) || ((XhcReadExtCapReg (Xhc, Xhc->DebugCapSupOffset) & 0xFF) != XHC_CAP_USB_DEBUG) ||
    ...
    //
    //If reset successfully, check every port warm reset.
    //
    PortNum = Xhc->HcSParams1.Data.MaxPorts;
    DEBUG ((DEBUG_INFO, "Port Number : %d\n", PortNum));
    if (!EFI_ERROR (Status)) {
      for (Index = 0; Index < PortNum; Index++) {
        //
        //Check every port warm reset.
        //
        //
        //Warm reset if the following conditions are met at the same time.
        //1.ROS = 0(PORTSC bit0 = 0)
        //2.RW1CS = 0(PORTSC bit1 = 0)
        //3.RWS = 6(PORTSC bit8:5 = 6,inactive)
        //
        DEBUG ((DEBUG_INFO, "Check warm reset port[%d]\n", Index));
        Offset = (UINT32) (XHC_PORTSC_OFFSET + (0x10 * Index));
        State = XhcReadOpReg (Xhc, Offset);
        if (((State & XHC_PORTSC_PLS) >> 5 == 6) &&
             ((State & XHC_PORTSC_PED) >> 1 == 0) &&
             ((State & XHC_PORTSC_CCS) == 0)) {
          State |= XHC_PORTSC_WPR;
          XhcWriteOpReg (Xhc, Offset, State);
          Status1 = XhcWaitOpRegBit (Xhc, Offset, XHC_PORTSC_PRC, TRUE, XHC_GENERIC_TIMEOUT);
          if (EFI_ERROR (Status1)) {
            DEBUG ((DEBUG_ERROR, "Port[%d] warm reset failed!\n", PortNum));
          } else {
            DEBUG ((DEBUG_INFO, "Port[%d] warm reset success\n", PortNum));
          }
        }
      }
    }
