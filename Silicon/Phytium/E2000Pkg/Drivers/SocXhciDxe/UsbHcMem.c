  ...
  )
{
  ...
  gBS->FreePages ((EFI_PHYSICAL_ADDRESS)Block->BufHost, EFI_SIZE_TO_PAGES (Block->BufLen));
  ...
  )
{
  ...
  if (Alignment > EFI_PAGE_SIZE) {
    ...
    Memory = (UINTN)(AlignedMemory + EFI_PAGES_TO_SIZE (Pages));
  ...
  )
{
  ...
  //                  HostAddress
  //                  );
  Status = gBS->FreePages ((EFI_PHYSICAL_ADDRESS)HostAddress, Pages);
  ASSERT_EFI_ERROR (Status);
}
