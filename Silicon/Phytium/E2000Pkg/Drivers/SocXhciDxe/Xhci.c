...
#include "Xhci.h"
//#define PCI_BDF(b,d,f)  ((b) << 20 | (d) << 15 | (f) << 12)
//
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  MicroSecondDelay (CONTROLLER_TRANSFER_DELAY_BEFORE);
  Status = XhcTransfer (
             Xhc,
             DeviceAddress,
             Endpoint,
  ...
  DEBUG ((EFI_D_INFO, "%a(), %d, *DataLength : %d\n", __FUNCTION__, __LINE__, *DataLength));
  UINT32  Index1;
  for (Index1 = 0; Index1 < *DataLength; Index1++) {
    DEBUG ((EFI_D_INFO, "Data[%d] : %x\n", Index1,*((UINT8*)Data + Index1)));
  }
  ...
  )
{
  ...
  if (CompareGuid (Dev->Type, &gPsuUsb3NonDiscoverableDeviceGuid)) {
    Status = EFI_SUCCESS;
  } else {
  ...
  )
{
  ...
  Xhc->RegBase               = Dev->Resources[0].AddrRangeMin;

  DEBUG ((EFI_D_INFO, "Xhc->RegBase : %x\n", Xhc->RegBase));
  DEBUG ((EFI_D_INFO, "%a(), %d\n", __FUNCTION__, __LINE__));
  ReleaseNumber = 0xF1;
  ...
  //MmioWrite8 (0x40000000 + PCI_BDF (3, 0, 0) + 4, 0x7);
  DEBUG ((EFI_D_INFO, "%a(), %d\n", __FUNCTION__, __LINE__));
  //
  ...
  Xhc->CapLength        = (UINT8) XhcReadCapReg (Xhc, XHC_CAPLENGTH_OFFSET);
  ...
  )
{
  ...
  //
  DEBUG ((EFI_D_INFO, "%a(), %d\n", __FUNCTION__, __LINE__));
  Xhc = XhcCreateUsbHc (Dev, HcDevicePath, OriginalPciAttributes);
  DEBUG ((EFI_D_INFO, "%a(), %d\n", __FUNCTION__, __LINE__));
