...
EFIAPI
AhciReadReg (
  IN EFI_SATA_CONTROLLER_PRIVATE_DATA  *Ahc,
  IN UINT32                            Offset
  )
{
  UINT32    Data;
  ...
  Data = 0;
  Data = MmioRead32 (Ahc->RegBase + Offset);
  ...
  )
{
  ...
  if (CompareGuid (Dev->Type, &gSocSataNonDiscoverableDeviceGuid)) {
    Status = EFI_SUCCESS;
  } else {
  ...
  )
{
  ...
  //
  Private->Signature              = SOC_SATA_CONTROLLER_SIGNATURE;
  Private->RegBase                = Dev->Resources[0].AddrRangeMin;
  ...
  //
  Data32 = AhciReadReg (Private, R_AHCI_PI);
  ...
  //
  Data32 = AhciReadReg (Private, R_AHCI_CAP);
