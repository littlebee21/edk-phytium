...
DC_SYNC  mDcSyncTable[PHY_GOP_MAX_MODENUM] = {
  ...
  {968, 800, 840, 848, 1, 1300, 1280, 1288, 1294, 1}              //800x1280 50Hz
};
...
DP_SYNC mDpSyncTable[PHY_GOP_MAX_MODENUM] = {
  ...
  {968, 1300, 0, 0, 8, 6, 800, 1280, 128, 12, 1, 1}               //800x1280 50Hz
};
...
PHY_GOP_MODE mPhyGopMode[PHY_GOP_MAX_MODENUM] = {
  ...
  {800, 1280, 32, 60, PHY_GOP_800_1280}
};

/**
  Get dcdp config form share memory.
  ...
  )
{
  ...
  Temp = Address;
  Version = MmioRead32 (Temp);
  DEBUG ((DEBUG_INFO, "Share Memory Address : %llx, Version : %x\n", Address, Version));
  Temp += 4;
  Value = MmioRead32 (Temp);
  DEBUG ((DEBUG_INFO, "Enable : %x\n", Value));
  for (I = 0; I < DPDC_PATH_NUM; I++) {
    Private->DpIsUsed[I] = (Value >> I) & 0x1;
    DEBUG ((DEBUG_INFO, "Dp[%d] Used : %d\n", I, Private->DpIsUsed[I]));
  }
  Temp += 4;
  Value = MmioRead32 (Temp);
  DEBUG ((DEBUG_INFO, "Is Edp : %x\n", Value));
  for (I = 0; I < DPDC_PATH_NUM; I++) {
    Private->IsEdp[I] = (Value >> I) & 0x1;
    DEBUG ((DEBUG_INFO, "Edp[%d] Used : %d\n", I, Private->IsEdp[I]));
  }
  Temp += 4;
  Private->InitHor = MmioRead32 (Temp);
  Temp += 4;
  Private->InitVer = MmioRead32 (Temp);
  DEBUG ((DEBUG_INFO, "Init hor : %d, ver : %d\n", Private->InitHor, Private->InitVer));
}
        ...
        if (This->DpIsHpdOn[Index] == DP_HPD_OFF) {
          ...
          if(This->DpConnect[Index] == SINK_CONNECT) {
            if (1 == This->IsEdp[Index]) {
              ...
              //  if (EFI_SUCCESS == EdpBgLightControl (This, Index, 1, 50)) {
  ...
  )
{
  ...
  DcDpGetConfig (Dev->Resources[2].AddrRangeMin, Private);
  Width = Private->InitHor;
  Height = Private->InitVer;
  ...
  Private->GopDevicePath = DevicePath;
  Private->DcCtrlBaseAddr = Dev->Resources[0].AddrRangeMin + DC_CTRL_BASE_OFFSET;
  Private->DcChBaseAddr = Dev->Resources[0].AddrRangeMin + DC_CHANNEL_BASE_OFFSET;
  Private->DpChBaseAddr = Dev->Resources[0].AddrRangeMin + DP_CHANNEL_BASE_OFFSET;
  Private->DpPhyBaseAddr = Dev->Resources[1].AddrRangeMin + DP_PHY_BASE_OFFSET;
