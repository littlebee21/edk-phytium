...
#define FRONT_PAGE_KEY_POWEROFF               0x1003
...
)
{
  ...
  if (Status == EFI_SUCCESS) {
    if (PasswordConfigData.UserPasswordEnable == 1 || PasswordConfigData.AdminPasswordEnable == 1) {
      while (1) {
        ReadString (L"Enter Password", TempStr);
        if (PasswordConfigData.UserPasswordEnable == 1 && StrCmp (TempStr, PasswordConfigData.UserPassword) == 0) {
          ...
          if (--Chance == 0) {
            ...
            UnicodeSPrint (ChanceStr, sizeof (ChanceStr), L"You have %d chance left", Chance);
            do {
              CreateDialog (&Key, L"Password Incorrect", ChanceStr,L"Press ENTER to continue", NULL);
  ...
  )
{
  ...
  if (QuestionId != FRONT_PAGE_KEY_CONTINUE &&
      ...
      QuestionId != FRONT_PAGE_KEY_POWEROFF) {
    return FALSE;
  }
  ...
  )
{
  HiiCreateActionOpCode (
    StartOpCodeHandle,
    FRONT_PAGE_KEY_POWEROFF,
    STRING_TOKEN (STR_POWEROFF_STRING),
    STRING_TOKEN (STR_POWEROFF_STRING_HELP),
    EFI_IFR_FLAG_CALLBACK,
    0
    );
}
