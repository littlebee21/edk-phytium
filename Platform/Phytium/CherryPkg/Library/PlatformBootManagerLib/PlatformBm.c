  ...
  )
{
  ...
  EFI_STATUS                Status;
  DEBUG ((EFI_D_INFO, "Find Phytium Soc Device!\n"));
  ...
  if (CompareGuid (Device->Type, &gPsuUsb3NonDiscoverableDeviceGuid) ||
      CompareGuid (Device->Type, &gSocSataNonDiscoverableDeviceGuid) ||
      CompareGuid (Device->Type, &gSocDcDpNonDiscoverableDeviceGuid) ||
      CompareGuid (Device->Type, &gPhytiumUsb2NonDiscoverableDeviceGuid)) {
    return TRUE;
  }
  return FALSE;
  ...
  )
{
  ...
  //
  //Find all phytium soc devices and connect them.
  //
  FilterAndProcess (&gEdkiiNonDiscoverableDeviceProtocolGuid, IsPhytiumSocDevice , Connect);
  ...
  //
  // Add the hardcoded short-form PS2 keyboard device path to ConIn.
  //
  EfiBootManagerUpdateConsoleVariable (ConIn,
		(EFI_DEVICE_PATH_PROTOCOL *)&mKeyboardVendorDevicePath, NULL);
...
/**
  Create all signal to connect all drivers.
  ...
  )
{
  ...
  //PlatformRegisterFvBootOption (&gUefiShellFileGuid, L"UEFI Shell", 0, &F3);
  ...
  )
{
  ...
  Print (L"Press [F2] or [ESC]to show boot menu options.\n");
